...
##   PATH      follows typed NavGraph waypoints (walk/step/drop/jump/climb/wall)
...
func _update_lod() -> bool:
	var far := _player == null or global_position.distance_to(_player.global_position) > activation_range
...
func _process_chase(delta: float) -> void:
	...
	if flat_dist <= attack_range and absf(to_player.y) < 2.0 and not _on_wall_surface():
	...
	# Does direct steering suffice, or do we need the smart path?
	var needs_path := absf(to_player.y) > 1.6 or _stuck_timer > 0.8
	if needs_path and _try_request_path():
		_finish_move(delta)
		return

	# Wall walkers: player above and a face in front -> just climb it.
	if can_wall_walk and to_player.y > 1.6 and _try_attach_wall():
		_finish_move(delta)
		return

	_steer_move(_player.global_position, delta)
	_finish_move(delta)


## Crawling on a wall/ceiling toward the player; drop when they are below.
func _process_wall_chase(delta: float) -> void:
	var to_player := _player.global_position - global_position
	if to_player.y < -1.0 and _surface_normal.dot(to_player.normalized()) > 0.25:
		_detach_wall()
	else:
		_steer_move(_player.global_position + Vector3.UP * 0.5, delta, 0.85)
	_finish_move(delta)


func _process_path(delta: float) -> void:
	if _player == null or _path.is_empty() or _path_i >= _path.size():
		_clear_path()
		_process_chase(delta)
		return
	...
	var to_player := _player.global_position - global_position
	if Vector2(to_player.x, to_player.z).length() < 5.0 and absf(to_player.y) < 1.4:
		_clear_path()
		_process_chase(delta)
		return

	# Player drifted away from this path's goal -> repath on cadence.
	if _repath_timer <= 0.0 and _path_goal.distance_to(_player.global_position) > 4.0:
		_try_request_path()
	...
	match move:
		...
		NavGraph.Edge.CLIMB:
			...
			if flat < 1.7 and to_wp.y > 0.3 and is_on_floor():
				_begin_climb(wp_pos)
				_finish_move(delta)
				return
...
func _begin_jump(target: Vector3) -> void:
...
func _begin_climb(target: Vector3) -> void:
...
## Asks the NavGraph for a route to the player (budgeted per frame).
## Returns true when a usable path was found and PATH state was entered.
func _try_request_path() -> bool:
	if _graph == null or _player == null or not _graph.can_query():
		return false
	if _repath_timer > 0.0:
		return false
	_repath_timer = 1.1 + randf() * 0.5
	var path := _graph.find_path(global_position, _player.global_position, _profile)
	if path.is_empty():
		return false
	_path = path
	_path_i = 0
	_path_goal = _player.global_position
	_state = State.PATH
...
## Attaches to a steep face ahead (toward `hint`, or toward the player).
func _try_attach_wall(hint: Vector3 = Vector3.INF) -> bool:
	...
	if hint != Vector3.INF:
		...
		toward = Vector3(_player.global_position.x - global_position.x, 0.0, _player.global_position.z - global_position.z)

...
If `vendor/big-globe/` is empty, a human with network access should run:

```bash
./voxel-platform-handoff/scripts/fetch_big_globe_refs.sh
```

This clones Big Globe docs and key reference files into `vendor/big-globe/`. See [vendor/big-globe/MANIFEST.json](vendor/big-globe/MANIFEST.json).
